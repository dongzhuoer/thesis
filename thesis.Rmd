...
```{r geo-data-type, fig.cap="GEO 数据库中不同类型的数据分布", fig.width=11, fig.height=5.5}
cowplot::plot_grid(
    pie_plot(geo_method) + guides(fill = guide_legend(title = '类型')),
    pie_plot(geo_curate) + guides(fill = guide_legend(title = '经过校对')),
    labels = "AUTO", label_y = 0.85, label_size = 18
)
```
...
```{r lem4-promote-breast-cancer, fig.cap="LEM4 导致三苯氧胺耐药性的分子机制", out.width='70%'}
knitr::include_graphics('image/lem4-promote-breast-cancer.png')
```
...
```{r probe-to-symbol, fig.cap="将探针对应到 HUGO 基因符号（以通过Ensembl Gene ID 为例）", out.width='100%'}
knitr::include_graphics('image/probe-to-symbol.png')
```
...
```{r rGEO-performance, fig.cap="rGEO 的表现", fig.dim=c(10,4)}
gpl_plot <- tribble(
  ~old_type,    ~type,   ~n,   
  "known",      "可以处理", 2626L,
  "sequence",   "仅探针序列", 986L, 
  "unknown",    "无法处理", 783L, 
  "non-coding", "非编码基因", 473L
) %>% arrange(desc(n)) %>% pie_plot
...
gpl_gse_plot <- cowplot::plot_grid( 
	gpl_plot + theme(legend.position = "none"),
	gse_plot + theme(legend.position = "none"),
    hjust = -1, vjust = 2, labels = 'AUTO', label_y = 0.95, label_size = 16
) 

cowplot::plot_grid(
    gpl_gse_plot, 
    cowplot::get_legend(gse_plot + scale_fill_discrete(name = '类型')),
    rel_widths = c(1, .2)
)
```
...
```{r qGSEA-usage, fig.cap="qGSEA 的使用方法", out.width='100%'}
knitr::include_graphics('image/qGSEA-usage.jpeg')
```
...
```{r significant-geneset-per-dataset, fig.cap="各个集合在不同数据集中的整体表现", fig.dim=c(10, 9)}
ggplot(read_rds('data/significant_per_dataset.rds')) + 
    geom_density(aes(x = n)) + 
	geom_blank(aes(n*1.03)) + 
    scale_y_continuous(breaks = NULL) + 
    facet_wrap(~collection, scales = 'free') + 
    ylab('密度') + 
	xlab(expression(paste('该数据集中与', italic('LEM4'), '的表达量显著相关的基因集数目'))) + 
    theme_classic(16, 'WenQuanYi Zen Hei') + 
...
```{r significant-dataset-per-geneset, fig.cap="各个集合中前 10 位正相关和前 10 位负相关的基因集", fig.dim=c(8, 9)}
read_rds('data/gsea_summary.rds') %>% 
	group_by(collection) %>% arrange(desc(n)) %>% 
	group_by(collection, correlation) %>% arrange(desc(n)) %>% slice(1:10) %>% ungroup() %>% 
	mutate(correlation = ifelse(correlation == 'positive', '正相关', '负相关')) %>% 
	mutate(collection = str_remove(collection, '\\.all')) %>%
	ggplot() + 
        geom_boxplot(aes(x = collection, y = n), show.legend = F) + 
	    facet_wrap(~correlation, nrow = 2) + coord_flip() + 
        xlab('集合') + 
	    ylab(expression(paste('该基因集表现为与', italic('LEM4'), '的表达量显著相关的数据集数目'))) + 
        theme_classic(16, 'WenQuanYi Zen Hei') + 
	    theme(axis.title = element_text(size = 16)) + 
        theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
```
...
```{r geneset-overlap, fig.cap="h 集合中第 6 至 8 位基因集均与某些前 5 位基因集存在明显重合", out.width='100%'}
knitr::include_graphics('image/geneset-overlap.jpeg')
```
...
![图 S1 qGSEA 的界面，图中显示的是选择运行模式的界面](image/qGSEA-homepage.png)


![图 S2 qGSEA 的错误提示消息](image/qGSEA-error-hint.png)
...
![图 S3 h 集合中其它基因集的重合分析，与图7形成对照](image/geneset-overlap-control.jpeg)
...
![图 S4 在 javaGSEA 中制作表型文件](image/javaGSEA-make-cls.jpeg)
